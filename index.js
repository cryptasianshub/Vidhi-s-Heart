window.requestAnimationFrame = window.requestAnimationFrame || (function () {
   ...
   window.isDevice = /android|webos|iphone|ipad|ipod|blackberry|iemobile|opera mini|windows phone/i.test((navigator.userAgent|| window.opera ).toLowerCase());
   var loaded = false;
   var init =function (){
    if (loaded) return;
    loaded = true;
    ...
    var canvas = document.getElementById("vidhis-heart");
    ...
    for (i = 0; i < heartPointsCount; i++){
        ...
        e[i] = {
            vx: 0, vy: 0,
            R: 2, speed: ran() +5,
            q: ~~(ran() * heartPointsCount),
            D: 2 * (i % 2) - 1,
            force: 0.25 * ran() + 0.75,
            f: "hsla(0, " + ~~(40 * ran() + 60) + "%, " + ~~ (60 * ran() + 20) +"%, 0.5)",
            trace: []
        };
